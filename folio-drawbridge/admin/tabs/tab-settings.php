...
function folio_drawbridge_render_tab_settings(): void {
	...
	<script>
	var folioDrawbridgeKeyNonce = <?php echo wp_json_encode( wp_create_nonce( 'folio_drawbridge_generate_key_preview' ) ); ?>;
	var folioDrawbridgeAjaxUrl  = <?php echo wp_json_encode( admin_url( 'admin-ajax.php' ) ); ?>;
	...
	function folioDrawbridgeFetchKey() {
		...
		var body = new URLSearchParams({ action: 'folio_drawbridge_generate_key_preview', _wpnonce: folioDrawbridgeKeyNonce });
		fetch(folioDrawbridgeAjaxUrl, { method: 'POST', body: body,

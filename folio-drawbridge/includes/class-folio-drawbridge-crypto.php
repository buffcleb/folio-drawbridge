 ...
 *                 or overridden by defining FOLIO_DRAWBRIDGE_MASTER_KEY (64 hex chars) in
 ...
 *
 * Priority: FOLIO_DRAWBRIDGE_MASTER_KEY constant → wp_options fallback (generated on first call).
 */
function folio_drawbridge_get_master_key(): string {
	if ( defined( 'FOLIO_DRAWBRIDGE_MASTER_KEY' ) ) {
		$hex = FOLIO_DRAWBRIDGE_MASTER_KEY;
		if ( strlen( $hex ) === 64 && ctype_xdigit( $hex ) ) {
			return hex2bin( $hex );
		}
		// Misconfigured constant — log and fall through to DB key.
		error_log( 'Folio Drawbridge: FOLIO_DRAWBRIDGE_MASTER_KEY is defined but is not a valid 64-character hex string. Falling back to database key.' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log -- deliberate one-line warning so admins notice a misconfigured key constant.
	}

	$hex = get_option( 'folio_drawbridge_master_key' );
	if ( ! $hex || strlen( $hex ) !== 64 ) {
		$hex = bin2hex( random_bytes( 32 ) );
		// autoload=false: this option is only fetched when actually needed.
		update_option( 'folio_drawbridge_master_key', $hex, false );
	}
 ...
 */
function folio_drawbridge_derive_vault_key( string $vault_salt ): string {
	return hash_hmac( 'sha256', $vault_salt, folio_drawbridge_get_master_key(), true );
}

/**
 * Generates a random 32-byte hex vault salt for use in key derivation.
 */
function folio_drawbridge_generate_vault_salt(): string {
...
define( 'FOLIO_DRAWBRIDGE_STREAM_CHUNK', 1048576 );
 ...
 * all at once.
 *
 * Each intermediate chunk of FOLIO_DRAWBRIDGE_STREAM_CHUNK bytes is encrypted without
 ...
 */
function folio_drawbridge_encrypt_file( string $source_path, string $dest_path, string $vault_salt ) {
	...
	$key        = folio_drawbridge_derive_vault_key( $vault_salt );
	$iv         = random_bytes( 16 );
	$current_iv = $iv;
	$block      = 16;
	$success    = true;
	...
	while ( ! feof( $in ) ) {
		$plain_chunk = fread( $in, FOLIO_DRAWBRIDGE_STREAM_CHUNK );
 ...
 *
 * This is the streaming counterpart to folio_drawbridge_encrypt_file(). It must be called
 * AFTER all HTTP headers have been sent and before any other output. It never
 * holds more than two FOLIO_DRAWBRIDGE_STREAM_CHUNK buffers in memory simultaneously.
 *
 * $plaintext_size (from the folio_drawbridge_files.file_size column) is required so the
 ...
 * file length is a multiple of FOLIO_DRAWBRIDGE_STREAM_CHUNK.
 *
 ...
 * @param string $iv_hex        Hex-encoded IV from the folio_drawbridge_files record.
 * @param int    $plaintext_size Original file size in bytes (stored in DB).
 * @return bool True on success, false on failure.
 */
function folio_drawbridge_stream_decrypt_file(
	string $source_path,
	string $vault_salt,
	string $iv_hex,
	int    $plaintext_size
	...
	// Using ($plaintext_size - 1) ensures that an exact multiple of FOLIO_DRAWBRIDGE_STREAM_CHUNK
	// gives one fewer intermediate chunk (the full last chunk carries the padding).
	$intermediate_chunks = (int) ( ( $plaintext_size - 1 ) / FOLIO_DRAWBRIDGE_STREAM_CHUNK );

	// Compute the plaintext byte count in the last chunk (1–FOLIO_DRAWBRIDGE_STREAM_CHUNK).
	$last_plain_bytes = $plaintext_size - $intermediate_chunks * FOLIO_DRAWBRIDGE_STREAM_CHUNK;
	...
	// Process intermediate chunks — no padding, ciphertext == FOLIO_DRAWBRIDGE_STREAM_CHUNK bytes.
	for ( $i = 0; $i < $intermediate_chunks; $i++ ) {
		$cipher_chunk = fread( $in, FOLIO_DRAWBRIDGE_STREAM_CHUNK );

		if ( $cipher_chunk === false || strlen( $cipher_chunk ) !== FOLIO_DRAWBRIDGE_STREAM_CHUNK ) {
			$success = false;
			break;
		}
 ...
 */
function folio_drawbridge_decrypt_file_to_path(
	string $source_path,
	string $vault_salt,
	string $iv_hex,
	int    $plaintext_size,
 ...
 */
function folio_drawbridge_generate_token( int $bytes = 32 ): string {
 ...
 */
function folio_drawbridge_generate_otp(): string {
 ...
 */
function folio_drawbridge_hash_otp( string $otp ): string {
 ...
 */
function folio_drawbridge_verify_otp( string $otp, string $hash ): bool {

...
3. Copy the key into `wp-config.php` as `define('FOLIO_DRAWBRIDGE_MASTER_KEY', '...');`.
...
| **Drawbridge Admin** | `folio_drawbridge_manage_vaults` | Full Folio Drawbridge admin panel — all tabs, vault inspector, audit export, settings, Users tab |
...
3. `folio_drawbridge_send_otp()` validates the email matches the invite, generates a 6-digit OTP (via `random_int`), hashes it with `wp_hash_password`, and emails the plaintext code.
4. Recipient submits the OTP → `folio_drawbridge_verify_otp_for_share()` checks the hash, enforces the attempt limit, marks the OTP used.
...
Accessible at **Folio Drawbridge** (requires `manage_options` or `folio_drawbridge_manage_vaults` capability).
...
| Users | Manage Drawbridge Admins and Vault Users; grant, promote, demote, revoke; user search |
...
The `[folio_drawbridge_vaults]` shortcode renders equivalent functionality on any front-end page.
...
**Admin vault overview** (Drawbridge Admins only):
...
Encrypted files are written to `wp-content/uploads/folio-drawbridge-vaults/{vault_id}/{random}.enc`. The root is protected by `.htaccess` (`Deny from all`). Files are never served directly — all downloads go through PHP, which decrypts on the fly.

Chunked upload staging files live in `wp-content/uploads/folio-drawbridge-chunks/` and are cleaned up by the hourly cron.
...
| Table | Purpose |
|---|---|
| `{prefix}folio_drawbridge_vaults` | Vault records — owner, status, salt, expiry |
| `{prefix}folio_drawbridge_files` | File metadata — original name, stored name, IV, size, MIME |
| `{prefix}folio_drawbridge_shares` | Share links — recipient, token, download count, expiry |
| `{prefix}folio_drawbridge_otps` | OTP records — hash, attempt count, expiry |
| `{prefix}folio_drawbridge_audit` | Immutable event log |
...
### 1.2.0
- **Multi-file upload** — file inputs now accept `multiple` files. Files upload sequentially through the existing chunked system with per-file progress rows. Available in both the admin vault inspector and the `[folio_drawbridge_vaults]` shortcode.
...
- **Per-user storage quota** — set a per-user MB ceiling in Settings → Storage Quotas. Enforcement happens server-side at upload time; WordPress and Drawbridge admins are exempt.
...
- **DB version migration** — added `FOLIO_DRAWBRIDGE_DB_VERSION` constant and `folio_drawbridge_maybe_upgrade_db()` for safe, idempotent schema updates via `dbDelta()` on `plugins_loaded`.
...
- **Help tab updates** — all admin panel and user dashboard contextual help tabs updated to reflect Drawbridge Admin roles, sortable columns, dashboard widgets, and inline editing capabilities.
...
### 1.0.2
- **Drawbridge Admin user type** — new `folio_drawbridge_manage_vaults` capability grants non-WordPress-administrator users full access to the Folio Drawbridge admin panel. WordPress administrators continue to have full access implicitly. Promote, demote, and revoke from the redesigned Users tab.
- **Users tab redesign** — replaced WP role column with two sections: Drawbridge Admins and Vault Users. Search panel shows current Drawbridge status with contextual action buttons.
...
- **Role-based access (Users tab)** — grant and revoke the `folio_drawbridge_use_vaults` capability per user from the admin panel.
- **Bug fix** — `folio_drawbridge_max_download_limit` and `folio_drawbridge_max_expiry_days` settings were incorrectly clamped to a minimum of 1, preventing the "no ceiling" (0) value from being stored.
